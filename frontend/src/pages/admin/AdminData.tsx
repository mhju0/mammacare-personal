...
const SEVERITY_COLORS = ["#86EFAC", "#FDE68A", "#FDBA74", "#FCA5A5", "#CBD5E1"];
const STATUS_LABELS: Record<string, string> = {
  planned: "예정",
  done: "완료",
  skipped: "건너뜀",
};
const CHART_PRIMARY = "#93C5FD";
const CHART_GREEN = "#86EFAC";
const CHART_YELLOW = "#FDE68A";
        ...
        >
          {options.map((o) => (
            <button
              ...
              className={`w-full px-3 py-2.5 text-sm text-left font-medium transition-colors hover:bg-[#FAFAFA]/70 whitespace-nowrap ${
            ...
            <>
              <div className="bg-card border border-border rounded-2xl p-5">
                ...
                <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
                  <div className="h-full rounded-full bg-[#86EFAC] transition-all" style={{ width: `${data?.testing_completion_rate ?? 0}%` }} />
                </div>
              </div>
              <div className="bg-card border border-border rounded-2xl p-5">
                <div className="flex items-center justify-between mb-3">
                ...
                <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
                  <div className="h-full rounded-full bg-[#93C5FD] transition-all" style={{ width: `${data?.schedule_completion_rate ?? 0}%` }} />
                </div>
              </div>
            </>
          )}
            ...
            {loading ? <Skeleton className="h-64" /> : (data?.top_allergy_ingredients.length ?? 0) === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={250}>
                ...
                >
                  ...
                  <Tooltip contentStyle={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 12, fontSize: 13 }} />
                  <Bar dataKey="count" name="확진 수" fill="#FCA5A5" radius={[0, 4, 4, 0]} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </ChartCard>
            ...
            {loading ? <Skeleton className="h-52" /> : (
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={data?.baby_age_distribution ?? []} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <YAxis tick={{ fontSize: 11 }} stroke="var(--muted-foreground)" allowDecimals={false} />
                  <Tooltip contentStyle={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 12, fontSize: 13 }} />
                  <Bar dataKey="count" name="아기 수" fill="#C4B5FD" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </ChartCard>
            ...
            {loading ? <Skeleton className="h-52" /> : (
              <ResponsiveContainer width="100%" height={200}>
                ...
                >
                  ...
                  <Bar dataKey="avg_height" name="avg_height" fill="#FDBA74" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </ChartCard>
